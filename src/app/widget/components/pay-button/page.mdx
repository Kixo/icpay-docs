...
export const sections = [
  ...
  { title: 'X402 up-to (agentic)', id: 'x402-upto' },
...
- **`x402UptoSkipSettlementWait?: boolean`** — when `true` with up-to (EVM): no long poll; `onSuccess` may receive `status: 'authorized_pending_settlement'`; progress shows a banner + pending settlement step. See [Agentic X402 up-to payments](/agentic-x402#progress-skip).
...
Full flow, progress behavior, webhook **`x402_upto_authorization_received`**, and code examples: **[Agentic X402 up-to payments](/agentic-x402)** and **[Webhooks](/webhooks#event-x402-upto-authorization-received)**.
