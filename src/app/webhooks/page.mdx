...
export const sections = [
  ...
  { title: 'x402_upto_authorization_received', id: 'event-x402-upto-authorization-received' },
...
| `x402_upto_authorization_received` | **X402 up-to (EVM):** the client called **`POST /sdk/public/payments/intents/x402/upto/confirm`** and ICPay persisted the signed payment header on the intent. Use this to **sync your backend** with the wallet authorization *before* secret-key settlement and `payment.completed`. See [Agentic X402](/agentic-x402#webhook-x402-upto-authorization). |
...
- **type**: One of the event types in the table above (e.g. `payment.completed`, `x402_upto_authorization_received`).
...
## x402_upto_authorization_received

**Triggered when:** A publishable-key client successfully confirms an **X402 up-to** authorization for **EVM** — i.e. after **`POST /sdk/public/payments/intents/x402/upto/confirm`** persists the signed X402 header on the payment intent (and updates metadata such as `icpay_x402_upto_max_amount_atomic`). **No on-chain settlement has run yet**; that still happens when you call **`POST /sdk/payments/x402/upto/settle`** with the secret key (or via `icpay.protected.settleX402Upto`).

**Why it matters:** This is the **server-side hook** that lines up with the widget’s **`icpay-x402-upto-submitted`** window event and with **`x402UptoSkipSettlementWait`** flows where the browser does not poll until `completed`. Subscribe in **Settings → Webhook Endpoints** so your backend can start metered work, mark a job as “authorized”, or store `paymentIntentId` idempotently **before** `payment.completed`.

**data.object** (payment intent snapshot — not the same as every `payment_intent.created` field set; aligns with confirm response context):
...
```json
{
  ...
  "metadata": {
    "orderId": "job-123",
    "icpay_x402_upto_max_amount_atomic": "100000"
  },
  "intentCode": 123456
}
```
...
| Field | Type | Description |
|-------|------|-------------|
| id | string | Payment intent id (same as `paymentIntentId`). |
| object | string | Always `payment_intent` for this event. |
| paymentIntentId | string | Intent id — correlate with `onX402UptoIntent` / client callbacks. |
| status | string | Intent status after confirm (often still `requires_payment` or `processing` until settlement). |
| amount | string | Intent amount in smallest token unit (authorization context). |
| amountUsd | number \| null | Fiat display hint when present on the intent. |
| ledgerCanisterId | string \| null | Token ledger reference. |
| payer | string \| null | EVM payer address when verified from the header. |
| metadata | object | Intent metadata including up-to confirm hints. |
| intentCode | number \| null | Intent code for matching on-chain / internal flows. |

**Typical sequence with up-to:** `payment_intent.created` (optional) → **`x402_upto_authorization_received`** → later `payment.created` / `payment.updated` as settlement progresses → **`payment.completed`** when funds are finalized.
...
app.post('/webhooks/icpay', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  switch (event.type) {
    ...
    case 'x402_upto_authorization_received': /* sync: header saved; start job / await settle */ break
