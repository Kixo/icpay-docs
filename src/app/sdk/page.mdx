...
- `chainId` (optional, UUID): Target chain for the payment intent. Use this when directing payments to another supported chain.
...
- Payments:
  ...
  - `subscribePaymentIntentStatus({ paymentIntentId, onUpdate, onError?, intervalMs? })` → Subscribe to one payment intent via SSE.
  - `waitForPaymentIntentCompletion(paymentIntentId)` → Convenience helper that waits for terminal status using SSE first, then polling fallback.
